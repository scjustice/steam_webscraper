...
#            yield SplashRequest(url, self.parse, endpoint='render.html', args={'wait':0.5},)
    ...
    def parse(self, response):
        ...
        #browse_by_tag_urls = [(x,'https://store.steampowered.com/search/?sort_by=Released_DESC&tags={}&category1=998&supportedlang=english'.format(x)) for x in tag_list]
        browse_by_tag_urls = [(x,'https://store.steampowered.com/search/?tags={}&category1=998&supportedlang=english'.format(x)) for x in tag_list]
    ...
    def parse_tag_browse_list(self, response):
        ...
        genre_browse_url_list = ['https://store.steampowered.com/search/?tags={}&category1=998&page={}'.format(tag_id, x) for x in range(1, last_page+1)] 
    ...
    def parse_tag_browse_page(self, response):
        ...
        for game in game_list:
            ...
            if price == None:
                ...
                print('Title = {}, original price = {}, price = {}, game id = {}, url = {}'.format(title, orig_price, price, game_id, detail_url))
                #yield Request(url=detail_url, meta={'title':title, 'price':price, 'orig_price':orig_price}, callback=self.parse_game_detail)
                yield SplashRequest(url=detail_url, args={'wait':0.5, 'image':0,'lua_source':lua_script_detail}, endpoint='execute', callback=self.parse_game_detail, meta={'title':title, 'price':price, 'orig_price':orig_price})

            elif price.strip() == '':
                continue
            else:
                price = price.strip().strip('$')
                print('Title = {}, price = {}, game id = {}, url = {}'.format(title, price, game_id, detail_url))
            print('*' * 50)
#            yield Request(url=detail_url, meta={'title':title, 'price':price}, callback=self.parse_game_detail)
            yield SplashRequest(url=detail_url, args={'wait':0.5, 'image':0,'lua_source':lua_script_detail}, endpoint='execute', callback=self.parse_game_detail, meta={'title':title, 'price':price})
    ...
    def parse_game_detail(self, response):
        ...
        tag_list = list(map(lambda x: x.strip(), response.xpath('//div[@class="glance_tags popular_tags"]/a/text()').extract()))
        description = response.xpath('//div[@class="game_description_snippet"]/text()').extract_first().lstrip().rstrip()
        #Probably need to do a try for percent_pos and total reviews -- Otherwise set these to N/A

        #Handle games with no reviews
        test_for_no_reviews = response.xpath('//div[@class="summary column"]/\
                text()').extract_first().strip().find('No user') >= 0
        ...
        if test_for_no_reviews:
            ...
            review_text = response.xpath('//span[@class="nonresponsive_hidden responsive_reviewdesc"]/text()').extract_first().strip()
        #Handle games with some reviews but not enough for percent_pos
            if review_text.find('Need') >= 0:
                print('Not enough reviews for a % positive')
                percent_pos = 'N/A'
                total_reviews = re.search('(\d+) user', response.xpath('//span[@class=\
                        "game_review_summary not_enough_reviews"]/text()').extract_first()).group(1)
            else: 
        #Handle games with large number of reviews over a long amount of time
                print('Enough reviews for % positive')
                total_reviews = response.xpath('//div[@class="summary column"]/span[@class="responsive_hidden"]/text()')\
                    .extract()[-1].strip().strip('()').replace(',','')
                percent_pos = re.search('(\d+)%', response.xpath('//span\
                    [@class="nonresponsive_hidden responsive_reviewdesc"]/text()').extract_first()).group(1)
#        try:
